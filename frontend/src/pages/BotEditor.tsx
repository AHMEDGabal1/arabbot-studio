...
import { ArrowLeft, Save, Sparkles, Bot as BotIcon, Send, Sliders, Users, ShieldAlert } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import toast from 'react-hot-toast';
import { createBot, getBot, updateBot } from '../lib/api';
import type { BotCreate } from '../types';
import SpecialistAgentsManager from '../components/SpecialistAgentsManager';
...
export default function BotEditor() {
  ...
  // Active Tab State for Existing Bots
  const [activeTab, setActiveTab] = useState<'general' | 'agents' | 'guardrails'>('general');
    ...
    <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display text-3xl font-extrabold text-navy-900">{isNew ? 'Create New AI Bot' : (form.name || 'Edit Bot Settings')}</h1>
            <p className="text-xs text-ash-400 font-arabic" dir="rtl">تعديل شخصية البوت، الوكلاء المتخصصون، وإعدادات الحماية والأمان</p>
          </div>
        </div>
      </div>
      ...
      {/* Tabbed Navigation Bar (Shown for Existing Bots) */}
      {!isNew && (
        <div className="border-b border-sand-200 bg-white/60 backdrop-blur-md rounded-2xl p-1.5 shadow-xs border flex items-center gap-1">
          <button
            type="button"
            onClick={() => setActiveTab('general')}
            className={`flex-1 py-2.5 px-4 rounded-xl font-body text-xs font-bold transition-all flex items-center justify-center gap-2 ${
              activeTab === 'general'
                ? 'bg-navy-900 text-white shadow-sm'
                : 'text-ash-400 hover:text-navy-900 hover:bg-sand-100/70'
            }`}
          >
            <Sliders className="w-4 h-4" />
            <span>Tab 1: General & Persona</span>
            <span className="text-[10px] font-arabic font-normal opacity-80" dir="rtl">(عام والشخصية)</span>
          </button>
          ...
          >
            ...
            <span>Tab 2: Specialist Agents</span>
            <span className="text-[10px] font-arabic font-normal opacity-80" dir="rtl">(الوكلاء المتخصصون)</span>
          </button>
          ...
          >
            ...
            <span>Tab 3: Safety & Guardrails</span>
            <span className="text-[10px] font-arabic font-normal opacity-80" dir="rtl">(قواعد الأمان)</span>
          </button>
        </div>
      )}

      {/* Tab 1: General & Persona (Dual Grid) */}
      ...
      {!isNew && activeTab === 'agents' && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          <SpecialistAgentsManager botId={id!} />
        </motion.div>
      )}

      {/* Tab 3: Safety & Guardrails */}
      {!isNew && activeTab === 'guardrails' && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          <GuardrailsManager botId={id!} />
        </motion.div>
      )}
    </motion.div>
  );
